...
$(document).ready(function(){
  ...
  var set_question = function(res){
	...
	res.p = ('000' + res.p).slice(-3);
        $('.cell-info').append($('<span></span>').html('<a href="http://ckhis.ck.tp.edu.tw/~ljm/amis-mp/'+res.p+'.jpg" target="_blank">查看整頁</a>'));
        $('#unclear').show();
      });
  };
  ...
  function setProgress(data) {
    if(data['cnt'] != undefined) {
      var percent = Math.round(10000 * data['cnt'] / 27621) / 100;
      $('.bar').css('width', '' + percent + '%');
      $('#progress_text').text('已完成 ' + data['cnt'] + ' / 27621 (' + percent + '%)');
    }
  }
